...
char* base64Decode(const char* in, int inlength , int *resultSize) 
{
	if(inlength == 0)inlength = strlen(in);
	...
	unsigned char* out = new unsigned char[inlength];//(unsigned char*)strDupSize(in); // ensures we have enough space
	int k = 0;
	int const jMax = inlength - 3;
